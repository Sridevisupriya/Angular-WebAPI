...
namespace com.tweetapp.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<bool> UpdateUser(string userId, CreateUserDto userDetails)
        {
            try
            {
                var user = _mapper.Map<Users>(userDetails);
                bool respone = await _user.UpdateUser(userId , user);
                return respone;
            }
            catch (Exception ex)
            {
        ...
        public async Task<IEnumerable<ViewUserDto>> GetUsersByIdAsync(string id)
        {
            try
            {
                var users = await _user.GetUsersByUserIdAsync(id);
                var userDetail = new List<ViewUserDto>();
                foreach (var user in users)
                {
                    userDetail.Add(_mapper.Map<ViewUserDto>(user));
